...
import com.exataid.apontamentoplantio.telas.adapters.SincAdapter;
...
import com.exataid.apontamentoplantio.utils.Sync;
...
public class SincronizarActivity extends AppCompatActivity {
    ...
    private static List<LogSync> listaLogSyncCadastros, listaLogSyncApontamentos;
    ...
    private final BroadcastReceiver AtualizarDataUltimoSync = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                APDatabase.getInstance(context).getSincDAO().inserir(s).get();
                if (listaSync.size() > 2000) {
                    APDatabase.getInstance(context).getSincDAO().excluir(listaSync).get();
                }
                carregarBotoes();
                carregarUltimaSinc();
                carregarListener();
    ...
    void setarRecyclerListaItens() {
        ...
        if (listaLogSyncApontamentos != null) {
            SincAdapter listaAdapter = new SincAdapter(this, listaLogSyncApontamentos);
    ...
    private void carregarUltimaSinc() {
        ...
        try {
            sinc = APDatabase.getInstance(this).getSincDAO().buscarAtivo().get();
            listaSync = APDatabase.getInstance(this).getSincDAO().buscaTodos().get();
            listaURL = APDatabase.getInstance(this).getConfigWSDAO().buscarUm().get();
            listaLogSyncCadastros = APDatabase.getInstance(this).getLogSyncDAO().buscarTodasPorTipo("CADASTROS").get();
            listaLogSyncApontamentos = APDatabase.getInstance(this).getLogSyncDAO().buscarTodasPorTipo("APONTAMENTOS").get();
